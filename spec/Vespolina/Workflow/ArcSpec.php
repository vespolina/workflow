...
use Vespolina\Workflow\NodeInterface;
...
class ArcSpec extends ObjectBehavior
{
    ...
    function it_should_add_self_to_tokenable_from(NodeInterface $from)
    {
        $this->setFrom($from);
        $from->addOutput($this)->shouldHaveBeenCalled();
    }

    function it_should_add_self_to_tokenable_to(NodeInterface $to)
    {
        $this->setTo($to);
